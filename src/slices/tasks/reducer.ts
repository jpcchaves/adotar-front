...
const TasksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(addCardData.fulfilled, (state: any, action: any) => {
      const existingTaskList = state.tasks.find((kanbanList: any) => kanbanList.id === action.payload.kanId);
      if (existingTaskList) {
        ...
        if (task.id === action.payload.kanId) {
          return {
            ...
            cards: task.cards.map((card: any) =>
              card.id.toString() === action.payload.id.toString() ? { card, ...action.payload } : card,
            ),
          };
        }
        return task;
